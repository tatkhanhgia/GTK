...
import { HomepageFeaturedContent } from '@/components/sections/homepage-featured-content'
import { HomepageStartHereSection } from '@/components/sections/homepage-start-here-section'
import {
...
export default async function HomePage({ params }: Props) {
  ...
  const [authorProfile, postsResult, productsResult] =
    await Promise.all([
  ...
  return (
    <main className="flex flex-col">
      <HomepageEntryLoader locale={loc} />
      ...
      >
        <HeroInteractionFrame>
          ...
          <div className="relative z-10 flex flex-col items-center">
            <h1 className="mb-6 max-w-4xl font-heading text-3xl font-bold sm:text-4xl md:text-5xl">
              {t('hero.title')}{' '}
            ...
            <div className="mt-6 flex max-w-2xl flex-wrap justify-center gap-2">
              {[
                ...
                'Next.js',
                'Payload CMS',
                loc === 'vi' ? 'sản phẩm số' : 'digital products',
              ].map((topic) => (
                <span
                  key={topic}
                  className="rounded-full border border-border/70 bg-card/55 px-3 py-1 text-sm text-secondary-foreground"
                >
                  {topic}
                </span>
              ))}
      ...
      <ScrollReveal preset="section">
        <HomepageStartHereSection locale={loc} />
      </ScrollReveal>
      ...
      <HomepageFeaturedContent
        ...
        postsTitle={t('featuredPosts')}
        productsTitle={t('featuredProducts')}
      />
