...
import { cancelAdminAiAction } from '@/lib/admin-ai/admin-ai-confirmation-service'
...
export async function POST(request: Request) {
  ...
  try {
    await cancelAdminAiAction(payload, user, getId(await request.json()))
    ...
    const message = error instanceof AdminAiError ? error.message : 'Could not cancel action.'
