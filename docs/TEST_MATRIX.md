...
| Admin AI provider chat | Payload admins can save encrypted AI provider profiles, chat through OpenAI-compatible providers, persist/reopen/delete chat sessions, and confirm write tools before mutation | yes | partial | no | no | implemented | `tests/lib/admin-ai/*`, `tests/api/admin-ai-*.test.ts`, `npx tsc --noEmit`, focused Vitest run |
| Admin AI file uploads | Payload admins can attach Markdown/HTML/TXT files to AI chat, with DB-backed chunks, checksum dedupe, quota checks, delete semantics, and bounded provider context | yes | partial | component | no | implemented | `tests/lib/admin-ai/admin-ai-file-*.test.ts`, `tests/api/admin-ai-files.test.ts`, `tests/api/admin-ai-chat.test.ts`, `tests/admin/admin-ai-attachments.test.tsx`, `npx tsc --noEmit` |
