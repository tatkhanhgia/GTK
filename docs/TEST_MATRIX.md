...
| Admin email settings | Payload global stores provider-neutral mail config, encrypted Resend secret, same-provider env fallback, and provider adapter selection | yes | no | no | no | implemented | `tests/globals/email-settings-access.test.ts`, `tests/lib/email/email-settings-service.test.ts`, `tests/lib/email/send-email-provider-selection.test.ts`, `tests/lib/admin/site-user-admin-service.test.ts`, `npx tsc --noEmit` |
