...
The project goal is to use Harness v0 as a reusable operating layer that lets
humans and agents turn GTKBlog change requests into safe, validated work.
...
- Existing source folders, package scripts, tests, CI, database migrations, or
  infrastructure.
...
GTKBlog starts with an existing implementation. When the human provides a new
specification or feature request, treat it as input material, not as a permanent
operating manual. Use it to populate product docs, story packets, architecture
decisions, and validation expectations for the selected work.
